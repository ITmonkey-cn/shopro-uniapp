				...
				<view class="shop-modal page_box">
					...
					<view class="content_box">
						...
						<view class="buy-num-box x-bc">
							...
							<view class="num-step"><uni-number-box @change="changeNum" :step="1" :min="1" :currentSkuPrice.sync="currentSkuPrice" :goodsInfo="goodsInfo" :value="goodsNum"></uni-number-box></view>
						</view>
					</view>
		...
		changeDisabled(isChecked = false, pid = 0, skuId = 0) {	
			...
			if (isChecked) {
				...
				this.currentSkuArray.forEach(sku => {
					if (sku.toString() != '') {		// sku 为空是反选 填充的
						let index = noChooseSkuIds.indexOf(sku.toString());
						if (index >= 0) {		// sku 存在于 noChooseSkuIds
							noChooseSkuIds.splice(index, 1);
						}
					}
				});
