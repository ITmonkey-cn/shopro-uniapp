							...
							<view class="x-f">
								<view class="head-img-wrap">
									...
									<block v-if="platform !== 'H5'">
										<button v-if="platform === 'wxMiniProgram'" @tap="refreshWechatUser" class="cu-btn refresh-btn x-c">
	...
	methods: {
		...
		async refreshWechatUser(e) {
			...
			if (this.platform === 'wxOfficialAccount') {
				...
				let wechat = new Wechat();
				let token = await wechat.wxMiniProgramLogin(true)
