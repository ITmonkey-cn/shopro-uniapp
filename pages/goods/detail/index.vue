			...
			<view class="detail-content">
				<view class="goodes_detail_swiper-box">
					...
					<swiper class="carousel" circular @change="swiperChange" :autoplay="true">
						<swiper-item @tap="tools.previewImage(goodsInfo.images, swiperCurrent)" v-for="(img, index) in goodsInfo.images"
				...
				<shopro-sku v-model="showSku" :goodsInfo="goodsInfo" :buyType="goodsInfo.activity_type == 'seckill' || detailType === 'score' ? 'buy' : buyType"
				...
				<!-- 优惠券 -->
				<sh-coupon v-if="goodsInfo.coupons && goodsInfo.coupons.length && goodsInfo.activity_type !== 'seckill' && goodsInfo.activity_type !== 'groupon' && detailType !== 'score'"
				 :couponList="goodsInfo.coupons"></sh-coupon>
				<!-- 拼团人-->
				<sh-groupon v-if="goodsInfo.activity && goodsInfo.activity.type === 'groupon' && goodsInfo.activity.rules.team_card === '1' && detailType !== 'score'"
				 :grouponData="goodsInfo"></sh-groupon>
				<!-- 拼团玩法 -->
				<view v-if="goodsInfo.activity && (!!goodsInfo.activity.richtext_id) && goodsInfo.activity_type !== 'seckill'"
				 class="groupon-play x-bc px20" @tap="jump('/pages/public/richtext', { id: goodsInfo.activity.richtext_id })">
					<view class="x-f">
				...
				<view class="detail-right">
					...
					<view class="detail-btn-box x-ac" v-if="goodsInfo.activity && goodsInfo.activity.type === 'groupon'">
						...
						<view class="x-f" v-if="activityRules.status == 'ing'">
							...
							<button class="cu-btn tool-btn groupon-btn y-f" :style="goodsInfo.activity.rules.is_alone === '0' ? 'width:400rpx' : ''"
