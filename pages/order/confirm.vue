	...
	<view class="order-wrap">
		...
		<view class="u-m-b-10">
			...
			<block v-if="perGoodsList.length">
				...
				<!-- 发票 -->
				<view v-if="orderPre && Number(orderPre.invoice_amount)" class="u-flex u-row-between item-list u-p-20 border-top">
					<view class="item-title">申请发票</view>
					<view class="u-flex u-col-center" @tap="onInvoice">
						<text class="selected-invoice" style="font-size: 28rpx;color:#c4c4c4;">{{ selectedInvoice }}</text>
						<text class="u-iconfont uicon-arrow-right" style="color: #bfbfbf;"></text>
					</view>
				</view>
			</block>
		...
		<!-- 发票弹窗 -->
		<u-popup v-model="showInvoice" @close="showInvoice = false" safe-area-inset-bottom mode="bottom" :closeable="false" border-radius="20">
			<view class="invoice-modal page_box">
				<view class="invoice-head u-flex u-col-center">
					...
					<view class="head-nav u-flex u-col-center u-row-center" @tap="changeInvoiceType('company')">
						<text class="nav-title" :class="{ 'nav-title--active': invoiceType === 'company' }">企/事业单位</text>
				...
				<view class="invoice-content content_box">
					...
					>
						...
						<u-form-item class="u-p-r-20" v-if="invoiceType === 'company'" label-width="220" label="纳税人识别号:" prop="tax_no">
		...
		<u-popup v-model="showExpressType" @close="showExpressType = false" safe-area-inset-bottom mode="bottom" :closeable="false" border-radius="20">
			...
			<view class="express-type page_box">
						...
						<view class="" v-else>
							...
							<view class="express-bottom" @tap="checkProtocol">
								<u-checkbox active-color="#f0c785" :value="isProtocol">
									<view class="protocol">
										同意并接受
										<text class="protocol-text" @tap.stop="jump('/pages/public/richtext', { id: initStore.selfetch_protocol })">《到店自提服务协议》</text>
									</view>
								</u-checkbox>
							</view>
						</view>
				...
				<view class="express-type__bottom u-flex u-row-between u-p-b-20" v-show="expressTypeCur !== 'selfetch' || (expressTypeCur == 'selfetch' && lat)">
		...
		subOrder() {
			...
			that.$http(
				'order.createOrder',
				{
					...
					groupon_id: that.grouponId,
					invoice:
						Number(that.orderPre.invoice_amount) && that.selectedInvoice !== '不开具发票' ? { ...that.invoiceForm.model, amount: that.orderPre.invoice_amount } : {}
				},
				'提交中...'
			).then(res => {
