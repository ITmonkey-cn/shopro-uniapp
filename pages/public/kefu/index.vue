		...
		<view class="kefu_container">
			...
			<!-- #endif -->
				...
				<view :style="{height:'calc(100% - ' + wrapperHeight + 'px)'}" class="chat_wrapper">
					<scroll-view @scrolltoupper="wrapper_scrolltoupper" @tap="tap_scroll_view" :scroll-y="true" :scroll-with-animation="true"
				...
				<view class="write" :style="{bottom: writeBottom + 'px'}">
					<view class="widget_textarea">
						<textarea :adjust-position="false" :show-confirm-bar="false" :fixed="true" :focus="kefuMessageFocus" :auto-height="true"
						 maxlength="-1" @blur="kefu_message_blur" @input="textarea_input" :cursor-spacing="14" @focus="textarea_focus"
						 :value="kefuMessage" class="kefu_message"></textarea>
					</view>
		...
		methods: {
			...
			build_url: function(type = 'ws', res_path) {
				...
				switch(type) {
					...
					return (protocol + Config.baseURL + ':' + that.config.websocket_port + '/?modulename=index' + tokenStr + kefu_user).replace(
						/\|/g, '~');
					break;
					//初始化
					case 'initialize':
