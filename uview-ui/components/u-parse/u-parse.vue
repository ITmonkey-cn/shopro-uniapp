...
export default {
	...
	methods: {
		...
		setContent(html, append) {
			...
			if (append)
				this.$refs.web.evalJs("var b=document.createElement('div');b.innerHTML='" + html.replace(/'/g, "\\'") + "';document.getElementById('parser').appendChild(b)");
			else {
				html =
		...
		navigateTo(obj) {
			...
			if (!obj.id) weexDom.scrollToElement(this.$refs.web);
				...
				this.$refs.web.evalJs(
					'var pos=document.getElementById("' + obj.id + '");if(pos)post({action:"linkpress",href:"#",offset:pos.offsetTop+' + (obj.offset || 0) + '})'
				);
